...
import LiveCat3UpdateForm from "@/pages/Account/components/UpdateForm";
import {removeLiveCat3} from "@/pages/live/category/cat3/config/api/removeLiveCat3";
import {getLiveCat3} from "@/pages/live/category/cat3/config/api/getLiveCat3";
...
const TableList: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<API.LiveCat3>();
  const [selectedRowsState, setSelectedRows] = useState<API.LiveCat3[]>([]);
  ...
  const columns: ProColumns<API.LiveCat3>[] = [
    {
    ...
    },
    {
      ...
      dataIndex: 'option',
      valueType: 'option',
      // hidden: !access.upsetLiveCat3,
      render: (_, record) => [
        <>

          <LiveCat3UpdateForm actionRef={actionRef} record={record} key={"pages.searchTable.edit"}/>
  ...
  return (
    <PageContainer>
      <ProTable<API.LiveCat3, API.PageParams>
        ...
        toolBarRender={() => [
          <>
            <LiveCat3UpdateForm actionRef={actionRef} record={null} key={"pages.searchTable.new"}/>
            <Button
      ...
      {selectedRowsState?.length > 0 && (
        ...
        >
          <Button
            ...
            icon={<DeleteFilled/>}
            onClick={async () => {
              await removeLiveCat3(selectedRowsState);
              setSelectedRows([]);
        ...
        {currentRow?.name && (
          <ProDescriptions<API.LiveCat3>
            ...
            columns={columns.map(column => {
              ...
              return column as ProDescriptionsItemProps<API.LiveCat3>[];
            })}
          />
        )}
      </Drawer>
