...
import {upsetLiveCat3} from "@/pages/Account/config/api/upsetLiveCat3";
...
const LiveCat3UpdateForm: React.FC<API.UpdateFormProps> = ({actionRef, record}) => {
  const intl = useIntl();
    ...
    <>
      <ModalForm
        ...
        onFinish={async (value) => {
          ...
          const success = await upsetLiveCat3(newValue as API.LiveCat3);
          if (success) {
      ...
      >
        ...
        <ProFormText
          ...
          rules={[
            {required: true, message: intl.formatMessage({id: 'LiveCat3.updateForm.rule.name'})},
            {
              pattern: /^[a-zA-Z]{1,15}$/,
              message: intl.formatMessage({id: 'LiveCat3.updateForm.rule.name'})
            }
          ]}
        />
        <ProFormText
...
export default LiveCat3UpdateForm;
