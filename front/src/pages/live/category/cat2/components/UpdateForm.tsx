...
import {upsetLiveCat2} from "@/pages/Account/config/api/upsetLiveCat2";
...
const LiveCat2UpdateForm: React.FC<API.UpdateFormProps> = ({actionRef, record}) => {
  const intl = useIntl();
    ...
    <>
      <ModalForm
        ...
        onFinish={async (value) => {
          ...
          const success = await upsetLiveCat2(newValue as API.LiveCat2);
          if (success) {
      ...
      >
        ...
        <ProFormText
          ...
          rules={[
            {required: true, message: intl.formatMessage({id: 'LiveCat2.updateForm.rule.name'})},
            {
              pattern: /^[a-zA-Z]{1,15}$/,
              message: intl.formatMessage({id: 'LiveCat2.updateForm.rule.name'})
            }
          ]}
        />
        <ProFormText
...
export default LiveCat2UpdateForm;
