...
const UserUpdateForm: React.FC<API.UpdateFormProps> = ({actionRef, record}) => {
  const intl = useIntl();
    ...
    <>
      <ModalForm
        ...
        onFinish={async (value) => {
          ...
          const success = await upsetUser(newValue as API.User);
          if (success) {
      ...
      >
        ...
        <ProFormText
          ...
          rules={[
            {required: true, message: intl.formatMessage({id: 'User.updateForm.rule.name'})},
            {
              pattern: /^[a-zA-Z]{1,15}$/,
              message: intl.formatMessage({id: 'User.updateForm.rule.name'})
            }
          ]}
        />
        <ProFormText
