...
import {getRoleIdByUserId} from "@/pages/Account/config/api/getRoleIdByUserId";
import {upsetUserRole} from "@/pages/Account/config/api/upsetUserRole";

const AssignForm: React.FC<API.UpdateFormProps> = ({actionRef, userId}) => {
  const intl = useIntl();
    ...
    <>
      <ModalForm
        ...
        onFinish={async (value) => {
          const success = await upsetUserRole(userId, value.id);
          if (success) {
        ...
        request={async () => {
          let res = await getRoleIdByUserId([userId])
